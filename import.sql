-- ========================================
-- SOAL 3: Database Structure and Data
-- Database: testdb
-- ========================================
...
-- ALTER TABLE `hobi` ADD CONSTRAINT `fk_hobi_person` 
...
-- Query to show person with their hobbies
...
-- FROM person p 
-- LEFT JOIN hobi h ON p.id = h.person_id 
-- WHERE p.nama LIKE '%ana%'
-- GROUP BY p.id, p.nama, p.alamat;

-- Query to search by hobby
-- SELECT p.nama, p.alamat, GROUP_CONCAT(h.hobi SEPARATOR ', ') as hobi_list
-- FROM person p 
-- LEFT JOIN hobi h ON p.id = h.person_id 
-- WHERE p.id IN (SELECT person_id FROM hobi WHERE hobi LIKE '%Futsal%')
-- GROUP BY p.id, p.nama, p.alamat;
